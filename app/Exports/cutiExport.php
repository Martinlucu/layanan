...
class cutiExport implements FromCollection,WithHeadings
{
    ...
    */
    ...
    {$set = DB::table('setting')->where('id_set','1')->get();
        ...
        ->where('jenis','Cuti')->whereBetween('created_at', [$gjlawal, $gjlakhir])->wherein('status',['selesai','ditolak'])->orderBy('jurusan','asc')->get();
    } elseif(Carbon::now()>=$gnpawal && Carbon::now()<=$gnpakhir){
        $cuti = DB::table('dokumen')->select('nim','nama_mhs','email_mhs','jurusan','jenis','status','created_at','updated_at')
        ->where('jenis','Cuti')->whereBetween('created_at', [$gnpawal, $gnpakhir])->wherein('status',['selesai','ditolak'])->orderBy('jurusan','asc')->get();
    }
        return $cuti;
    }
